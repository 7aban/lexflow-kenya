...
export function Table({ columns, rows, empty, rowIds, rowProps }) { if (!rows.length) return <Empty title={empty} text="Once records exist, they will appear here." />; return <div style={styles.tableWrap}><table style={styles.table}><thead><tr>{columns.map(c => <th key={c}>{c}</th>)}</tr></thead><tbody>{rows.map((row, i) => <tr key={i} id={rowIds?.[i]} {...(rowProps?.[i] || {})}>{row.map((cell, j) => <td key={j} data-label={columns[j]}>{cell}</td>)}</tr>)}</tbody></table></div>; }
