...
export function Stat({ label, value, tone, icon: Icon }) { const colors = { navy: theme.navy600, gold: theme.gold, green: theme.green, red: theme.red }; const color = colors[tone] || theme.navy600; return <div style={{ ...styles.stat, borderLeftColor: color }}>{Icon ? <Icon size={20} style={{ background: `${color}16`, color, padding: 6, borderRadius: 8, width: 32, height: 32 }} /> : <i style={{ background: `${color}16`, color }}>{label.slice(0, 2).toUpperCase()}</i>}<span>{label}</span><strong>{value}</strong></div>; }
