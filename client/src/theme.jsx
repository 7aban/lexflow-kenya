...
export function resolveReadableTheme(input = {}) {
  const primaryColor = input.primaryColor || theme.navy800;
  const accentColor = input.accentColor || theme.gold;
  const backgroundColor = input.backgroundColor || '#F5F2EB';
...
export function StyleTag() { return <style>{`
  * { box-sizing: border-box; }
  ...
  table td { padding: 10px 12px; border-top: 1px solid var(--lf-card-border, var(--lf-border, #E5E7EB)); color: var(--lf-card-text, var(--lf-text, #101827)); background: var(--lf-card, #fff); vertical-align: middle; }
  table tr:nth-child(even) td { background: color-mix(in srgb, var(--lf-card, #fff) 96%, var(--lf-background, #FAFAFB)); }
  table tbody tr:hover td { background: color-mix(in srgb, var(--lf-accent, #D4A34A) 9%, var(--lf-card, #fff)); }
  label > span { color: var(--lf-card-muted, var(--lf-text-muted, #6B7280)); font-size: 11px; font-weight: 700; text-transform: uppercase; }
  .lf-app-shell .profile-tooltip span, body .profile-tooltip span { display: block; color: var(--lf-card-muted, var(--lf-text-muted, #697386)); font-size: 12px; margin-top: 3px; }
  .lf-logo-preview img { width: 42px; height: 42px; border-radius: 8px; object-fit: cover; border: 1px solid var(--lf-card-border, var(--lf-border, #E5E7EB)); }
  @media (max-width: 980px) {
  ...
  #root .lf-topbar-icon-btn:hover { background: color-mix(in srgb, var(--lf-on-header, #101827) 8%, transparent); color: var(--lf-on-header, #101827); }
  ...
  #root .lf-dash-panel { background: var(--lf-card, #fff); color: var(--lf-card-text, var(--lf-text, #101827)); border: 1px solid var(--lf-card-border, var(--lf-border, #DDD8CE)); border-radius: 10px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,.04); }
  ...
  #root .lf-dash-pipeline-row { display: flex; align-items: center; gap: 13px; padding: 9px 18px; border-bottom: 1px solid var(--lf-card-border, var(--lf-border, #DDD8CE)); cursor: pointer; transition: background .15s; background: transparent; color: var(--lf-card-text, var(--lf-text, #101827)); border-left: 0; border-right: 0; border-top: 0; width: 100%; text-align: left; }
  #root .lf-dash-pipeline-row:last-of-type { border-bottom: none; }
  #root .lf-dash-pipeline-row:hover { background: color-mix(in srgb, var(--lf-accent, #D4A34A) 12%, var(--lf-card, #fff)); }
  ...
  @media (min-width: 768px) {
    ...
    #root .lf-dash-table td { padding: 11px 18px; font-size: 13px; color: var(--lf-card-text, var(--lf-text, #101827)); border-bottom: 1px solid var(--lf-card-border, var(--lf-border, #DDD8CE)); border-top: 0; background: var(--lf-card, #fff); vertical-align: middle; }
    #root .lf-dash-table tr:last-child td { border-bottom: none; }
    #root .lf-dash-table tr:nth-child(even) td { background: var(--lf-card, #fff); }
    #root .lf-dash-table tbody tr:hover td { background: color-mix(in srgb, var(--lf-accent, #D4A34A) 10%, var(--lf-card, #fff)); cursor: default; }
    #root .lf-dash-table tr.lf-dash-today-row td { background: color-mix(in srgb, var(--lf-accent, #D4A34A) 8%, var(--lf-card, #fff)); }
    #root .lf-dash-table tr.lf-dash-today-row td:first-child { border-left: 3px solid var(--lf-accent, #D4A34A); padding-left: 15px; }
  }
  @media (max-width: 980px) {
...
export const styles = {
  ...
  userCard: { display: 'grid', gridTemplateColumns: '30px 1fr auto', gap: 10, alignItems: 'center', padding: '12px 16px 16px', borderTop: '1px solid color-mix(in srgb, var(--lf-on-sidebar, #fff) 12%, transparent)' }, avatar: { width: 30, height: 30, borderRadius: 999, background: 'var(--lf-accent, var(--lf-button, #D4A34A))', color: 'var(--lf-on-accent, var(--lf-button-text, #fff))', display: 'grid', placeItems: 'center', fontWeight: 800, fontSize: 12, flexShrink: 0, overflow: 'hidden' }, avatarImg: { width: '100%', height: '100%', objectFit: 'cover', borderRadius: 999, display: 'block' }, userName: { fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 12.5, color: 'var(--lf-on-sidebar, #fff)' }, userRole: { color: 'color-mix(in srgb, var(--lf-on-sidebar, #fff) 44%, transparent)', fontSize: 9, textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 700, marginTop: 2 }, logout: { border: '1px solid color-mix(in srgb, var(--lf-on-sidebar, #fff) 18%, transparent)', color: 'var(--lf-on-sidebar, #fff)', background: 'transparent', borderRadius: 6, padding: '5px 8px', cursor: 'pointer', fontSize: 12 },
  main: { minWidth: 0, display: 'flex', flexDirection: 'column' }, topbar: { display: 'flex', alignItems: 'center', gap: 12, height: 54, padding: '0 28px', background: 'var(--lf-header-bg, #FFFFFF)', color: 'var(--lf-on-header, var(--lf-header-text, #101827))', borderBottom: '1px solid var(--lf-border, #DDD8CE)', position: 'sticky', top: 0, zIndex: 100, flexShrink: 0 }, topbarTitle: { display: 'flex', gap: 10, alignItems: 'center', minWidth: 0 }, mobileMenuButton: { display: 'none', minWidth: 42, minHeight: 38, border: '1px solid var(--lf-border, #DDD8CE)', borderRadius: 8, background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #1A3628)', fontWeight: 700, cursor: 'pointer', alignItems: 'center', justifyContent: 'center', padding: '8px 10px', flexShrink: 0 }, eyebrow: { color: 'var(--lf-accent, #D4A34A)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0 }, title: { margin: '2px 0 0', fontSize: 20, fontWeight: 700, lineHeight: 1.2 }, subtitle: { margin: '5px 0 0', color: 'var(--lf-text-muted, #697386)', maxWidth: 680 }, topActions: { display: 'flex', gap: 6, alignItems: 'center', marginLeft: 'auto' }, search: { width: 260, border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: '7px 11px', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', fontSize: 13 },
  topbarSearch: { flex: '1 1 auto', maxWidth: 400, minWidth: 0, position: 'relative', display: 'flex', alignItems: 'center', gap: 9, background: 'var(--lf-card, #F5F2EB)', border: '1px solid var(--lf-border, #DDD8CE)', borderRadius: 7, padding: '7px 13px', cursor: 'text', transition: 'border-color .15s ease, box-shadow .15s ease' }, topbarSearchIcon: { color: 'var(--lf-card-muted, var(--lf-text-muted, #6B7280))', flexShrink: 0, display: 'inline-flex' }, topbarSearchInput: { flex: 1, border: 'none', background: 'transparent', outline: 'none', fontSize: 13, color: 'var(--lf-card-text, var(--lf-text, #101827))', minWidth: 0, padding: 0, fontFamily: 'inherit' }, topbarKbd: { fontSize: 10, color: 'var(--lf-card-muted, #6B7280)', border: '1px solid var(--lf-border, #DDD8CE)', borderRadius: 4, padding: '1px 5px', fontFamily: 'monospace', whiteSpace: 'nowrap', flexShrink: 0 }, iconButton: { width: 34, height: 34, minWidth: 34, minHeight: 34, border: '1px solid transparent', borderRadius: 7, background: 'transparent', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'color-mix(in srgb, var(--lf-on-header, #6B7280) 68%, transparent)', position: 'relative', padding: 0, flexShrink: 0 }, iconBadgeDot: { position: 'absolute', top: 6, right: 7, width: 7, height: 7, borderRadius: '50%', background: 'var(--lf-danger, #B91C1C)', border: '1.5px solid var(--lf-card, #fff)', pointerEvents: 'none' },
  pageInner: { padding: '24px 28px 124px', display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0, flex: 1 }, pageHeader: { display: 'grid', gap: 3, marginBottom: 4 }, pageCrumb: { fontSize: 10, color: 'var(--lf-text-muted, #6B7280)', textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: 600 }, pageTitle: { margin: 0, fontSize: 24, fontWeight: 600, color: 'var(--lf-text, #101827)', lineHeight: 1.2, letterSpacing: -.2 }, pageSub: { margin: '2px 0 0', fontSize: 13, color: 'var(--lf-text-muted, #6B7280)', maxWidth: 720, lineHeight: 1.5 },
  dashBanner: { background: 'var(--lf-primary, #1A3628)', borderRadius: 10, padding: '18px 22px', display: 'flex', alignItems: 'center', gap: 14, color: 'var(--lf-on-primary, #fff)', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }, dashBannerIcon: { width: 42, height: 42, background: 'color-mix(in srgb, var(--lf-accent, #D4A34A) 22%, transparent)', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--lf-accent, #D4A34A)', flexShrink: 0, position: 'relative', zIndex: 1 }, dashBannerText: { flex: 1, minWidth: 0, position: 'relative', zIndex: 1 }, dashBannerLabel: { fontSize: 10, textTransform: 'uppercase', letterSpacing: 1.6, color: 'color-mix(in srgb, var(--lf-on-primary, #fff) 50%, transparent)', marginBottom: 3, fontWeight: 600 }, dashBannerSum: { fontSize: 14, color: 'color-mix(in srgb, var(--lf-on-primary, #fff) 84%, transparent)', lineHeight: 1.5 }, dashBannerSumStrong: { color: 'var(--lf-on-primary, #fff)', fontWeight: 600 }, dashBannerAmount: { fontSize: 27, fontWeight: 700, color: 'var(--lf-accent, #D4A34A)', whiteSpace: 'nowrap', position: 'relative', zIndex: 1, letterSpacing: -.3 },
  ...
  dashStatsGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0,1fr))', gap: 12 }, dashStatCard: { position: 'relative', overflow: 'hidden', background: 'var(--lf-card, #fff)', border: '1px solid var(--lf-card-border, var(--lf-border, #DDD8CE))', borderRadius: 10, padding: '16px 18px', display: 'grid', gap: 0, font: 'inherit', color: 'var(--lf-card-text, var(--lf-text, #101827))', textAlign: 'left', width: '100%' }, dashStatTopBar: { position: 'absolute', top: 0, left: 0, right: 0, height: 2.5, borderRadius: '10px 10px 0 0' }, dashStatHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }, dashStatLabel: { fontSize: 10.5, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--lf-card-muted, var(--lf-text-muted, #6B7280))', fontWeight: 600 }, dashStatIcon: { width: 30, height: 30, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center' }, dashStatValue: { fontSize: 29, fontWeight: 600, color: 'var(--lf-card-text, var(--lf-text, #101827))', lineHeight: 1, letterSpacing: -.5 }, dashStatValueSm: { fontSize: 20 }, dashStatNote: { fontSize: 11, color: 'var(--lf-card-muted, var(--lf-text-muted, #6B7280))', marginTop: 4 },
  dashPanelHeadTitle: { fontSize: 13, fontWeight: 600, color: 'var(--lf-card-text, var(--lf-text, #101827))', margin: 0 }, dashPanelHeadSub: { fontSize: 11.5, color: 'var(--lf-card-muted, var(--lf-text-muted, #6B7280))', margin: '1px 0 0' }, dashPipelineLabel: { width: 96, fontSize: 12.5, color: 'var(--lf-card-text, var(--lf-text, #101827))', flexShrink: 0 }, dashPipelineTrack: { flex: 1, height: 5, borderRadius: 3, overflow: 'hidden', background: 'color-mix(in srgb, var(--lf-card-border, #DDD8CE) 80%, transparent)' }, dashPipelineFill: { height: '100%', background: 'var(--lf-accent, #D4A34A)', borderRadius: 3 }, dashPipelineCount: { width: 24, fontSize: 12, fontWeight: 600, color: 'var(--lf-card-text, var(--lf-text, #101827))', textAlign: 'right', flexShrink: 0 }, dashPanelBody: { padding: '8px 0' }, dashPanelEmpty: { padding: 24, color: 'var(--lf-card-muted, var(--lf-text-muted, #6B7280))', textAlign: 'center', fontSize: 13 },
  pageStack: { display: 'grid', gap: 16 },   heroCard: { borderRadius: 10, background: 'linear-gradient(135deg, var(--lf-primary, #112219), var(--lf-sidebar, #1A3628))', color: 'var(--lf-on-primary, #fff)', padding: 24, display: 'flex', justifyContent: 'space-between', gap: 20, alignItems: 'end', boxShadow: theme.shadow }, heroKicker: { color: 'var(--lf-accent, var(--lf-button, #D4A34A))', fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }, heroFigure: { fontSize: 24, fontWeight: 700, color: 'var(--lf-accent, var(--lf-button, #D4A34A))' },
  ...
  statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }, stat: { position: 'relative', overflow: 'hidden', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, var(--lf-text, #101827))', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderLeft: '4px solid', borderRadius: 10, padding: 16, boxShadow: theme.shadow, display: 'grid', gap: 6 },
  ...
  card: { background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, var(--lf-text, #101827))', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 10, padding: '18px 20px', boxShadow: theme.shadow, minWidth: 0, transition: 'box-shadow .16s ease' }, cardHead: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 14 },
  formGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 12, alignItems: 'end' }, field: { display: 'grid', gap: 5 }, input: { width: '100%', border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: '7px 11px', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, var(--lf-text, #101827))', fontSize: 13 }, colorInput: { width: '100%', height: 36, border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: 4, background: 'var(--lf-card, #fff)' }, primaryButton: { border: 0, borderRadius: 6, padding: '7px 16px', background: 'var(--lf-primary, #1B3A5C)', color: 'var(--lf-on-primary, #fff)', fontWeight: 700, cursor: 'pointer', fontSize: 13 }, goldButton: { border: 0, borderRadius: 6, padding: '7px 16px', background: 'var(--lf-accent, #D4A34A)', color: 'var(--lf-on-accent, #101827)', fontWeight: 700, cursor: 'pointer', fontSize: 13 }, dangerButton: { border: 0, borderRadius: 6, padding: '7px 16px', background: 'var(--lf-danger, #B91C1C)', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 13 }, ghostButton: { border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: '7px 16px', background: 'var(--lf-card, #fff)', color: 'var(--lf-link, var(--lf-primary, #1B3A5C))', fontWeight: 700, cursor: 'pointer', fontSize: 13, textDecoration: 'none' }, actionGroup: { display: 'flex', flexWrap: 'wrap', gap: 6 }, actionMenuWrap: { position: 'relative', display: 'inline-flex', justifyContent: 'flex-end' }, actionMenuButton: { width: 30, height: 28, border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, background: 'var(--lf-card, #fff)', color: 'var(--lf-link, var(--lf-primary, #1B3A5C))', fontWeight: 900, cursor: 'pointer', lineHeight: 1 }, actionMenu: { position: 'fixed', zIndex: 2600, minWidth: 154, display: 'grid', gap: 2, padding: 6, background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, var(--lf-text, #101827))', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8, boxShadow: theme.shadowLift, animation: 'lfDropIn .14s ease-out' }, actionMenuItem: { width: '100%', border: 0, borderRadius: 6, padding: '8px 10px', background: 'transparent', color: 'var(--lf-card-text, var(--lf-text, #101827))', cursor: 'pointer', fontSize: 12, fontWeight: 700, textAlign: 'left' }, actionMenuItemDanger: { color: theme.red, background: theme.redBg }, tinyButton: { border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: '4px 10px', background: 'var(--lf-card, #fff)', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: 'var(--lf-link, var(--lf-primary, #1B3A5C))' }, dangerTinyButton: { border: '1px solid #FECACA', borderRadius: 6, padding: '4px 10px', background: theme.redBg, cursor: 'pointer', fontSize: 12, fontWeight: 700, color: theme.red },
  tableWrap: { overflowX: 'auto', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8 }, table: { width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 620 }, badge: { display: 'inline-flex', padding: '4px 8px', borderRadius: 999, fontWeight: 700, fontSize: 12 }, link: { color: 'var(--lf-link, #1B3A5C)', fontWeight: 700, textDecoration: 'none' }, meetingLink: { color: 'var(--lf-link, #1B3A5C)', fontWeight: 700, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }, videoBadge: { width: 22, height: 22, borderRadius: 6, display: 'inline-grid', placeItems: 'center', background: theme.blueBg, color: theme.blue, fontSize: 10, fontWeight: 900 }, joinButton: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, padding: '5px 10px', background: theme.green, color: '#fff', fontWeight: 800, fontSize: 12, textDecoration: 'none', whiteSpace: 'nowrap', boxShadow: '0 6px 14px rgba(4,120,87,.18)' }, mutedText: { color: 'var(--lf-card-muted, var(--lf-text-muted, #697386))' }, tableSelect: { border: '1px solid var(--lf-border, #E5E7EB)', borderRadius: 6, padding: '5px 8px', fontSize: 12, background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)' },
  ...
  matterButton: { width: '100%', display: 'grid', gap: 4, textAlign: 'left', border: 0, borderLeft: '3px solid transparent', borderBottom: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', padding: '12px 10px', cursor: 'pointer', borderRadius: 6 }, matterActive: { background: 'color-mix(in srgb, var(--lf-accent, #D4A34A) 10%, var(--lf-card, #F8FAFC))', borderLeft: '3px solid var(--lf-accent, #D4A34A)' }, detailStack: { display: 'grid', gap: 16, minWidth: 0 }, chips: { display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, color: 'var(--lf-card-muted, var(--lf-text-muted, #697386))' }, tabList: { display: 'flex', gap: 4, borderBottom: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))' }, tabButton: { border: 0, borderBottom: '2px solid transparent', background: 'transparent', color: 'var(--lf-card-muted, var(--lf-text-muted, #697386))', padding: '8px 12px', cursor: 'pointer', fontWeight: 700, fontSize: 13 }, tabActive: { color: 'var(--lf-link, #1B3A5C)', borderBottom: '2px solid var(--lf-accent, #D4A34A)' }, assistantPanel: { display: 'grid', gap: 12 }, assistantIntro: { display: 'grid', gap: 4, padding: 14, borderRadius: 10, background: 'color-mix(in srgb, var(--lf-card, #fff) 90%, var(--lf-background, #F8FAFC))', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', color: 'var(--lf-card-muted, #697386)' }, suggestionList: { display: 'grid', gap: 10 }, suggestionItem: { display: 'grid', gridTemplateColumns: '28px 1fr', gap: 10, alignItems: 'start', padding: '12px 14px', borderRadius: 10, border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', boxShadow: theme.shadow, lineHeight: 1.5 }, suggestionWarning: { background: theme.amberBg, borderColor: '#FDE68A', color: theme.amber }, suggestionGood: { background: theme.greenBg, borderColor: '#A7F3D0', color: theme.green }, suggestionIcon: { width: 28, height: 28, display: 'grid', placeItems: 'center', fontSize: 16 }, sub: { borderTop: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', paddingTop: 16 }, noteForm: { display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, marginBottom: 12 }, file: { marginBottom: 12, fontSize: 12 }, hoverName: { color: 'var(--lf-link, #1B3A5C)', fontWeight: 800, cursor: 'help' },
  empty: { minHeight: 136, display: 'grid', placeItems: 'center', textAlign: 'center', color: 'var(--lf-card-muted, var(--lf-text-muted, #697386))', gap: 6, padding: 18 }, emptyIcon: { width: 42, height: 42, borderRadius: 10, display: 'grid', placeItems: 'center', background: 'color-mix(in srgb, var(--lf-card, #fff) 86%, var(--lf-background, #F3F4F6))', color: 'var(--lf-link, #1B3A5C)', fontWeight: 800 }, skeletonGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 12 }, skeleton: { height: 120, borderRadius: 10, background: 'var(--lf-card, #fff)', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', boxShadow: theme.shadow, padding: 16, display: 'grid', gap: 10, alignContent: 'start' }, skeletonLineLarge: { height: 18, width: '55%', borderRadius: 6, background: 'color-mix(in srgb, var(--lf-card-border, #E5E7EB) 82%, transparent)', animation: 'lfPulse 1.4s ease-in-out infinite' }, skeletonLine: { height: 12, width: '80%', borderRadius: 6, background: 'color-mix(in srgb, var(--lf-card-border, #EEF2F7) 64%, transparent)', animation: 'lfPulse 1.4s ease-in-out infinite' }, skeletonLineShort: { height: 12, width: '42%', borderRadius: 6, background: 'color-mix(in srgb, var(--lf-card-border, #EEF2F7) 64%, transparent)', animation: 'lfPulse 1.4s ease-in-out infinite' },
  toast: { position: 'fixed', top: 22, right: 22, zIndex: 2000, display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', minWidth: 300, maxWidth: 420, padding: '12px 14px', borderRadius: 10, boxShadow: theme.shadowLift, border: '1px solid', animation: 'lfSlideIn .18s ease-out' }, toastClose: { border: 0, background: 'transparent', color: 'inherit', fontWeight: 900, cursor: 'pointer', fontSize: 16, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: 4 }, mobileDrawerLayer: { position: 'fixed', inset: 0, zIndex: 2800 }, mobileBackdrop: { position: 'absolute', inset: 0, border: 0, padding: 0, background: 'rgba(8,18,37,.48)', cursor: 'pointer' }, mobileDrawer: { position: 'absolute', top: 0, left: 0, bottom: 0, width: 304, maxWidth: 'calc(100vw - 44px)', background: 'var(--lf-sidebar, #112219)', color: 'var(--lf-on-sidebar, var(--lf-sidebar-text, #fff))', padding: 0, display: 'flex', flexDirection: 'column', overflowY: 'auto', boxShadow: '18px 0 42px rgba(8,18,37,.32)' }, mobileDrawerHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '16px 16px 12px', borderBottom: '1px solid color-mix(in srgb, var(--lf-on-sidebar, #fff) 12%, transparent)' }, mobileCloseButton: { minWidth: 38, minHeight: 38, border: '1px solid color-mix(in srgb, var(--lf-on-sidebar, #fff) 18%, transparent)', borderRadius: 8, background: 'transparent', color: 'var(--lf-on-sidebar, #fff)', cursor: 'pointer', fontWeight: 900, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }, modalBackdrop: { position: 'fixed', inset: 0, zIndex: 3000, display: 'grid', placeItems: 'center', padding: 24, background: 'rgba(15,27,51,.28)', backdropFilter: 'blur(4px)' }, modalCard: { width: 420, maxWidth: '100%', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 10, boxShadow: theme.shadowLift, padding: 18 }, modalHead: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 10 }, modalActions: { display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 18 }, alert: { borderRadius: 8, padding: 12, background: theme.blueBg, color: theme.blue, fontWeight: 700 }, alertDanger: { background: theme.redBg, color: theme.red },
...
loginCardForm: { width: '100%', maxWidth: 360, color: 'var(--lf-card-text, #101827)', background: 'var(--lf-card, #fff)', border: '1px solid var(--lf-card-border, #E8E2D6)', borderRadius: 14, padding: '32px 30px', boxShadow: '0 1px 2px rgba(17,34,25,.04), 0 18px 40px rgba(17,34,25,.06)' },
loginCardTitle: { margin: 0, fontSize: 27, fontWeight: 600, color: 'var(--lf-card-text, #101827)', fontFamily: "'Playfair Display', Georgia, 'Times New Roman', serif", letterSpacing: -.2, lineHeight: 1.15 },
...
loginInput: { width: '100%', border: '1px solid var(--lf-card-border, #E8E2D6)', borderRadius: 8, padding: '10px 12px', background: 'var(--lf-card, #fff)', fontSize: 13.5, color: 'var(--lf-card-text, #101827)' },
loginPrimaryButton: { border: 0, borderRadius: 9, padding: '12px 16px', background: 'var(--lf-primary, #112219)', color: 'var(--lf-on-primary, #fff)', fontWeight: 700, cursor: 'pointer', fontSize: 13.5, letterSpacing: .3, boxShadow: '0 8px 18px rgba(17,34,25,.18)' },
loginOauthButton: { width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '9px 14px', fontSize: 13, border: '1px solid var(--lf-card-border, #E8E2D6)', borderRadius: 8, cursor: 'pointer', background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', fontWeight: 600, transition: 'background .16s ease, border-color .16s ease' },
...
loginDemoHint: { marginTop: 12, color: '#9C9384', fontSize: 11.5, textAlign: 'center', letterSpacing: .2 },
  logoPreview: { display: 'flex', alignItems: 'center', gap: 10, minHeight: 36 }, formHelper: { color: 'var(--lf-card-muted, var(--lf-text-muted, #697386))', fontSize: 12, lineHeight: 1.4, alignSelf: 'center' }, clientShell: { minHeight: '100vh', background: 'var(--lf-background, #F5F7FA)', color: 'var(--lf-text, #101827)' }, clientHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, padding: '16px 24px', background: 'var(--lf-header-bg, var(--lf-card, #fff))', color: 'var(--lf-on-header, var(--lf-header-text, #101827))', borderBottom: '1px solid var(--lf-border, #E5E7EB)', boxShadow: theme.shadow }, clientBrand: { display: 'flex', alignItems: 'center', gap: 10 }, clientActions: { display: 'flex', alignItems: 'center', gap: 8 }, clientMain: { display: 'grid', gridTemplateColumns: '300px minmax(0,1fr)', gap: 16, padding: 24 }, clientMatterList: { minWidth: 0 }, clientStatusGrid: { display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }, clientDescription: { color: 'var(--lf-text-muted, #697386)', marginTop: 12 }, profileTooltip: { position: 'fixed', zIndex: 5000, pointerEvents: 'none', width: 260, display: 'grid', gridTemplateColumns: '42px 1fr', gap: 10, background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 10, padding: 12, boxShadow: theme.shadowLift }, tooltipAvatar: { width: 42, height: 42, borderRadius: 10, background: 'var(--lf-primary, #1B3A5C)', color: 'var(--lf-on-primary, #fff)', display: 'grid', placeItems: 'center', fontWeight: 900 },
  noticeAttachmentPreview: { gridColumn: '1 / -1', display: 'grid', gap: 8, padding: 10, border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8, background: 'color-mix(in srgb, var(--lf-card, #fff) 88%, var(--lf-background, #F8FAFC))' }, noticeAttachmentPreviewItem: { display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 10, alignItems: 'center', padding: '8px 10px', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8, background: 'var(--lf-card, #fff)' }, noticeAttachmentSummary: { display: 'flex', flexWrap: 'wrap', gap: 6 }, noticeList: { display: 'grid', gap: 12 }, noticeItem: { display: 'grid', gap: 10, padding: 14, border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8, background: 'var(--lf-card, #fff)', color: 'var(--lf-card-text, #101827)' }, noticeItemHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }, noticeTitle: { margin: 0, fontSize: 15, color: 'var(--lf-card-text, #101827)', lineHeight: 1.25 }, noticeBody: { margin: 0, color: 'var(--lf-card-text, #101827)', lineHeight: 1.55, whiteSpace: 'pre-wrap' }, noticeFileGrid: { display: 'grid', gap: 8 }, noticeFileLink: { display: 'grid', gridTemplateColumns: '42px minmax(0,1fr) auto', gap: 10, alignItems: 'center', padding: '9px 10px', border: '1px solid var(--lf-card-border, var(--lf-border, #E5E7EB))', borderRadius: 8, background: 'color-mix(in srgb, var(--lf-card, #fff) 88%, var(--lf-background, #F8FAFC))', color: 'var(--lf-link, #1B3A5C)', textDecoration: 'none', fontWeight: 700 }, noticeFileIcon: { width: 42, minHeight: 28, borderRadius: 6, display: 'grid', placeItems: 'center', background: theme.blueBg, color: theme.blue, fontSize: 10, fontWeight: 900 },
