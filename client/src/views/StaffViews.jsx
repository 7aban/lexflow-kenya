      ...
      {selectedInvoice && (
        <div style={{ ...styles.pageStack, marginTop: 16 }}>
          <Sub title={`Payments - ${selectedInvoice.number || selectedInvoice.id}`}>
            <div className="lf-invoice-cards"><Table columns={['Date', 'Receipt', 'Method', 'Reference', 'Amount', 'Receipt PDF']} rows={payments.map(payment => [payment.date || '-', payment.receiptNumber || '-', payment.method || '-', payment.reference || '-', kes(payment.amount), payment.receiptNumber ? <DownloadButton key={`${payment.id}-receipt`} label="Download" path={`/api/invoices/${selectedInvoice.id}/payments/${payment.id}/receipt.pdf`} filename={`${payment.receiptNumber}.pdf`} notify={notify} /> : '-'])} empty="No payments recorded yet." /></div>
