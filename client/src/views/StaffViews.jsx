...
export function Clients({ clients, matters, canManage, isAdmin = false, reload, notify, focus }) {
  ...
  useEffect(() => { api('/firm-settings').then(s => { setRetainerEnabled(Boolean(s?.moduleSettings?.retainerManagement)); setLedgerEnabled(Boolean(s?.moduleSettings?.retainerLedger)); }).catch(() => {}); }, []);
