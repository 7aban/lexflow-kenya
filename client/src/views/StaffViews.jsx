...
export function Tasks({ data, canManage, reload, notify, focus }) {
  ...
  async function submit(event) { event.preventDefault(); try { await api('/tasks', { method: 'POST', body: form }); setForm({ matterId: '', title: '', dueDate: '', assignee: '' }); notify({ type: 'success', message: 'Task added.' }); await reload(); } catch (err) { notify({ type: 'danger', message: err.message }); } }
  ...
  return <div className="lf-split-grid lf-task-split-grid" style={styles.splitGrid}><Card title="Add task" hint="Track pending work on a matter"><form onSubmit={submit} style={styles.formGrid}><div style={{ ...styles.formHelper, gridColumn: '1 / -1' }}>Tasks track work that still needs doing. Pick the matter, describe the work, then mark the task done when it is finished.</div><Field label="Matter"><select required style={styles.input} value={form.matterId} onChange={e => setForm({ ...form, matterId: e.target.value })}><option value="">Select matter</option>{data.matters.map(m => <option key={m.id} value={m.id}>{m.title}</option>)}</select></Field><Field label="Task title"><input required style={styles.input} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. File statement of defence" list="task-suggestions-nt" /></Field><datalist id="task-suggestions-nt"><option value="File pleadings" /><option value="Serve summons" /><option value="Draft agreement" /><option value="Prepare affidavit" /><option value="Attend court" /><option value="Legal research" /><option value="Client meeting" /><option value="Review documents" /><option value="Update client" /><option value="File application" /><option value="Prepare submissions" /><option value="Witness preparation" /><option value="Settlement discussions" /><option value="Obtain orders" /><option value="Taxation of costs" /></datalist><Field label="Due date / target date"><input type="date" style={styles.input} value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} /></Field><Field label="Assignee (optional)"><input style={styles.input} value={form.assignee} onChange={e => setForm({ ...form, assignee: e.target.value })} placeholder="e.g. Sarah Mwangi" /></Field><button style={styles.primaryButton}>Add task</button></form></Card><Card title="Task board" hint={`${filteredTasks.length} of ${allTasks.length} tasks`}><div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap', alignItems: 'center' }}><select aria-label="Filter tasks by status" value={taskFilter} onChange={e => setTaskFilter(e.target.value)} style={{ ...styles.input, width: 'auto', minWidth: 128 }}><option value="open">Open</option><option value="overdue">Overdue</option><option value="done">Done</option><option value="all">All</option></select><span style={{ fontSize: 12, color: '#697386' }}>Showing {filteredTasks.length} of {allTasks.length} tasks</span><button type="button" style={{ ...styles.ghostButton, fontSize: 12, padding: '6px 12px' }} onClick={exportTasks} disabled={!filteredTasks.length} title="Download the filtered tasks as a CSV file">Export CSV</button></div><TaskEditorList tasks={filteredTasks} matters={data.matters} canManage={canManage} editingTask={editingTask} setEditingTask={setEditingTask} saveTask={saveTask} toggle={toggle} confirmDelete={task => setConfirm({ title: 'Delete task?', message: 'Delete this task?', onConfirm: () => deleteTaskRecord(task) })} emptyTitle={taskEmpty[0]} emptyText={taskEmpty[1]} /></Card><ConfirmModal confirm={confirm} onClose={() => setConfirm(null)} /></div>;
}
