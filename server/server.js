...
app.get('/api/performance/advocates/:userId', requireAdmin, async (req, res) => {
  ...
  const [activeMatters, monthlyRows, recentTimeEntries] = await Promise.all([
    all(`SELECT m.id,m.title,m.reference,m.stage,(SELECT MIN(a.date) FROM appearances a WHERE a.matterId=m.id AND a.date>=?) nextCourtDate
      FROM matters m WHERE m.assignedTo=? AND m.stage<>'Closed' ORDER BY m.openDate DESC`, [today(), summary.fullName || '']),
    all(`SELECT substr(date,1,7) month, COALESCE(SUM(hours),0) hours, COALESCE(SUM(hours*rate),0) revenue
      FROM time_entries WHERE attorney=? AND date>=? GROUP BY substr(date,1,7) ORDER BY month`, [summary.fullName || '', startMonth]),
  ...
  res.json({ ...summary, activeMatterList: activeMatters, monthlyBreakdown, recentTimeEntries });
});
