module.exports = ({ get, all, documentListColumns, clientDocumentVisibilitySql, publicDocument, publicNotice }) => {
  async function getClientDashboardData(clientId, req) {
    ...
    const documents = matterIds.length ? await all(`SELECT ${documentListColumns()} FROM documents d LEFT JOIN folders f ON f.id=d.folderId WHERE d.matterId IN (${placeholders}) AND d.deletedAt IS NULL AND ${clientDocumentVisibilitySql('d')} ORDER BY d.date DESC`, [...matterIds, clientId || '']) : [];
    ...
    const noticeAttachments = noticeIds.length ? await all(`SELECT ${documentListColumns()} FROM documents d LEFT JOIN folders f ON f.id=d.folderId WHERE d.noticeId IN (${noticeIds.map(() => '?').join(',')}) AND d.deletedAt IS NULL AND COALESCE(d.clientVisible,0)=1 ORDER BY d.date DESC`, noticeIds) : [];
    const paymentProofs = await all('SELECT id,invoiceId,matterId,clientId,method,reference,amount,note,fileName,mimeType,size,createdAt,status,reviewedAt,reviewNote,paymentId FROM payment_proofs WHERE clientId=? ORDER BY createdAt DESC', [clientId || '']);
    return {
      client,
      matters,
