  ...
  async function advocatePerformanceRows() {
    ...
    for (const advocate of advocates) {
      ...
      const [matterStats, taskStats, timeStats, invoiceStats, courtStats, monthStats, weekStats] = await Promise.all([
        ...
        get(`SELECT
          COALESCE(SUM(hours),0) totalHours,
          COALESCE(SUM(CASE WHEN billable=1 THEN hours ELSE 0 END),0) billableHours,
          COALESCE(SUM(CASE WHEN billable=0 THEN hours ELSE 0 END),0) nonBillableHours,
          COALESCE(SUM(CASE WHEN billable=1 THEN hours*rate ELSE 0 END),0) billableAmount
        FROM time_entries WHERE attorney=?`, [name]),
        ...
        get(`SELECT
          COALESCE(SUM(hours),0) thisMonthHours,
          COALESCE(SUM(CASE WHEN billable=1 THEN hours ELSE 0 END),0) thisMonthBillableHours,
          COALESCE(SUM(CASE WHEN billable=1 THEN hours*rate ELSE 0 END),0) thisMonthRevenue
        FROM time_entries WHERE attorney=? AND date>=?`, [name, startMonth]),
        get('SELECT COALESCE(SUM(hours),0) last7Hours FROM time_entries WHERE attorney=? AND date>=?', [name, lastWeek]),
      ]);
      const billableAmount = Number(timeStats?.billableAmount || 0);
      rows.push({
        ...advocate,
        ...
        nonBillableHours: Number(timeStats?.nonBillableHours || 0),
        billableAmount,
        ...
        thisMonthBillableHours: Number(monthStats?.thisMonthBillableHours || 0),
  ...
  async function advocatePerformanceDetail(fullName) {
    ...
    const [activeMatters, monthlyRows, recentTimeEntries] = await Promise.all([
      ...
      all(`SELECT
          ...
          COALESCE(SUM(CASE WHEN billable=1 THEN hours ELSE 0 END),0) billableHours,
          COALESCE(SUM(CASE WHEN billable=0 THEN hours ELSE 0 END),0) nonBillableHours,
          COALESCE(SUM(CASE WHEN billable=1 THEN hours*rate ELSE 0 END),0) revenue
